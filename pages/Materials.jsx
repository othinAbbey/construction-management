...
//       <div>
//         <h6>
//           Are you a certified supplier?<span>Join Many of our suppliers</span>
//         </h6>
...
//           {selectedRegion2 && (
//             <select
//               onChange={(event) => handleDistrictChange(event, 2)}
//               value={selectedDistrict2}
//               className="w-1/3 p-2 border border-gray-300 rounded focus:outline-none focus:ring focus:border-blue-300"
//             >
//               <option value="Select District">Select District</option>
//               {regions
//                 .find((region) => region.name === selectedRegion2)
//                 ?.districts.map((district, index) => (
//                   <option key={index} value={district}>
//                     {district}
//                   </option>
...
//           {selectedDistrict2 && (
//             <select
//               onChange={(event) => handleMaterialChange(event, 2)}
...
import React, { useState } from "react";
import MaterialRateForm from "./MaterialRateForm"; // Adjust the path if necessary
import AddMaterialForm from "./AddMaterialForm"; // Adjust the path if necessary

const Materials = ({ regions, materials, addMaterial }) => {
  const [showAddForm, setShowAddForm] = useState(false);

  const handleAddMaterial = (e) => {
    e.preventDefault();
    // Logic to add material
    addMaterial(/* parameters here */);
  };

  return (
    <div>
      <MaterialRateForm regions={regions} materials={materials} />
      <button
